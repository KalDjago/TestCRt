...
app = FastAPI(title="TAM CRt Calculation Tool V5.2", version="0.5.2")
...
def build_master(frame, filename, vin_col, date_col, year_col, origin_col, crt_year, origin_filter):
    ...
    for row in frame.iter_rows(named=True):
        ...
        if vin in master:
            duplicate_count += 1
            continue
        sales_year = extract_year(row.get(year_col) if year_col else row.get(date_col))
        if sales_year is None:
            invalid_year += 1
            continue
        age = crt_year - sales_year
        if age < 1 or age > 8:
            outside_cohort += 1
            continue
        ...
        if normalized_filter and origin != normalized_filter:
            continue
        master[vin] = {"age": age, "sales_year": sales_year, "origin": origin, "total": 0, "same": 0}
    audit = {
        "file_name": filename,
...
def load_master_area(
    ...
    frame = read_selected_table(path, filename, required)
    master: dict[str, dict] = {}
    duplicate_count = invalid_year = blank_vin = outside_cohort = unmapped = 0
    normalized_filter = normalize(origin_filter)
    for row in frame.iter_rows(named=True):
        vin = normalize_vin(row.get(vin_col))
        if not vin:
        ...
        if normalized_filter and area != normalized_filter:
            continue
        master[vin] = {"age": age, "sales_year": sales_year, "origin": area, "total": 0, "same": 0}
    audit = {
        "file_name": filename,
        "master_rows": frame.height,
        "eligible_unique_vin": len(master),
...
@app.get("/health")
def health():
    return {"status": "ok", "version": "0.5.2", "engine": "V5 core plus optional Area enrichment"}
