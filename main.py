...
app = FastAPI(title="CRt Calculation Tool V5", version="0.5.0")
...
def merge_master(
    ...
    for vin, record in incoming.items():
        ...
        if (
            ...
            or existing["sales_year"] != record["sales_year"]
        ):
...
@app.get("/health")
def health():
    return {"status": "ok", "version": "0.5.0", "engine": "V2 conservative upgrade"}
...
async def calculate(
    ...
    try:
        ...
        for upload in master_files:
            ...
            paths.append(path)
            current_master, file_audit = load_master(
                path,
                upload.filename or "",
                master_vin,
                sales_date,
        ...
        if not master:
            raise ValueError(
                "No eligible unique VIN found for cohort 1Y-8Y. Check sales date/year, origin mapping, and filter."
            )
        ...
        backend_seconds = round(time.perf_counter() - started_at, 2)
        ...
        audit = {
            ...
            "blank_master_vin": sum(item["blank_master_vin"] for item in master_file_audits),
            "blank_sales_origin": sum(item["blank_sales_origin"] for item in master_file_audits),
            ...
            "backend_processing_seconds": backend_seconds,
        }
